        ...
        switch (paramType) {
            ...
            case UNION:
                ...
                if(!((UnionFunctionParam) functionParam).getUnionMemberParams().isEmpty()) {
                    Combo comboField = getComboField((UnionFunctionParam) functionParam, functionParam.getValue(), functionParam.getDescription());
                    builder.addFromTemplate(COMBO_TEMPLATE_PATH, comboField).addSeparator(ATTRIBUTE_SEPARATOR);
                }

//                Attribute unionAttr = new Attribute(functionParam.getValue(), displayName, INPUT_TYPE_STRING_OR_EXPRESSION,
//                        "", functionParam.isRequired(), functionParam.getDescription(), "",
//                        "", isCombo);
//                builder.addFromTemplate(ATTRIBUTE_TEMPLATE_PATH, unionAttr);
                ...
                for (FunctionParam member : unionMembers) {
                    ...
                    builder.addConditionalSeparator((unionMembers.indexOf(member) < unionMembers.size() - 1), ATTRIBUTE_SEPARATOR);
                }
                break;
            default:
...
//    }
    ...
    private static Combo getComboField(UnionFunctionParam unionFunctionParam, String paramName, String helpTip) {
        ...
        for (FunctionParam member : unionMembers) {
            String comboItem = member.getParamType().equals(RECORD) ? String.format("%s_%s",
                    member.getParamType(), member.getValue()) : member.getParamType();
            unionJoiner.add("\"" + comboItem + "\"");
        }
        String unionComboValues = unionJoiner.toString();
        String defaultValue = unionMembers.getFirst().getParamType().equals(RECORD) ? String.format("%s_%s",
                unionMembers.getFirst().getParamType(), unionMembers.getFirst().getValue()) : unionMembers.getFirst().getParamType();
        // Combo field for selecting the data type
        String comboName = String.format("%s%s", paramName, "DataType");
        return new Combo(comboName, comboName, INPUT_TYPE_COMBO,
                unionComboValues, defaultValue,unionFunctionParam.isRequired(), unionFunctionParam.getEnableCondition(), helpTip);
//        StringJoiner unionJoiner = new StringJoiner(",", "[", "]");
...
//                unionComboValues, !parameter, helpTip);
//        return comboField;
    }
